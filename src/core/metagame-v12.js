...
/**
 * Rank individual value by one transitive, cost-aware contribution metric.
 * Full five-slot budget reallocation remains the majority signal. Matched-slot
 * contribution can add at most 40%, and that weight shrinks as the card itself
 * consumes more of the deck budget. This lets cheap cards receive credit for
 * real slot impact without letting 70+ cost cards look essential merely
 * because four teammates were starved by their own cost.
 */
function compareIndividualContribution(left, right) {
  const leftContribution = metagameV12ContributionEvidence(left);
  const rightContribution = metagameV12ContributionEvidence(right);
  ...
  return [...ratings]
    ...
    .map((rating, index) => {
      ...
      return {
        ...
        individualRankingBasis: "cost-weighted-full-budget-plus-matched-slot",
      };
    });
}
