...
const report = {
  ...
  model: {
    ...
    scoringPolicy: "単体貢献順位とブラウザ候補priorは、対象キャラを禁止して全5枠を再最適化した機会勝率差を第一根拠にする。全5枠再最適化の平均機会勝率差が同値の候補同士だけ、同一4枠差し替えの安定補正後差を第2根拠に使う。実際の予算差がある場合は枠内差し替えで上書きしない。",
    costPolicy: "候補を外した際のコストを5枠全体で再配分し、全合法候補から作る共有基準デッキを比較対象へ追加する。高コストキャラは、そのコストを他4枠へ再投資した代替構成より十分に強い場合だけ高い単体コスパ評価を得る。",
