...
function compactCandidate(entry, position) {
  const contribution = metagameV12ContributionEvidence(entry);
  const browserPriorScore = Number.isFinite(contribution.hybridRobust)
    ? Math.min(1, Math.max(0, 0.5 + 0.5 * Math.tanh(contribution.hybridRobust / 0.15)))
  ...
  return {
    ...
    // Browser generation uses the same transitive cost-weighted hybrid metric
    // as published rankings. Full-budget opportunity remains the majority
    // signal, while cheap cards can retain bounded credit for true slot impact.
    m: rounded(contribution.hybridMean),
    r: rounded(contribution.hybridRobust),
