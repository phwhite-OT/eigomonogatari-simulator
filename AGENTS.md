...
Distributed V12 finalization has two execution tiers. Planner jobs may read the full durable checkpoint once, but prefill workers receive only the lightweight manifest/evaluator context and their assigned globally unique deck IDs; they must not download or hydrate the full `evaluatedDeckPool` merely to rediscover that the planner already marked those decks missing. Small waves use at most 19 shards so one physical 19-runner wave is enough; larger waves may use 38 shards to retain tail-balancing while still capping concurrent runners at 19.
...
Recovery across interrupted fanout waves must union reusable cache artifacts from **multiple recent runs**, not only the immediately previous run. Failed merge/push attempts can form chains where each generation contains only a subset of the original shard deltas; selecting only the newest run causes already-computed exact battles to be forgotten and needlessly recomputed. The planner therefore enumerates recent non-expired matching cache artifacts, downloads each run into a separate directory, and merges the union before deduplicating new work.

Durable V12 cache entries use a backward-compatible lossless compact encoding for `scenarioValues`. In-memory values remain ordinary JavaScript Number arrays. Checkpoint serialization stores exact decisive values 0 / 0.5 / 1 as one-byte tags and stores only other values as their exact Float64 bytes, with the whole tagged stream base64-encoded in `scenarioValuesPackedV1`. Hydration accepts legacy JSON arrays, the temporary all-Float64 base64 format, and the current tagged format, restoring the same Number values exactly. Any script that consumes `evaluatedDeckPool` must go through `hydrateMetagameV12EvaluationCache` rather than reading `.result.scenarioValues` directly.
