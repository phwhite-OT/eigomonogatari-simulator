...
test("V12 cost-weighted hybrid rewards real low-cost slot contribution", () => {
  const ranked = rankMetagameV12Characters([
    {
      id: "passenger",
      cost: 17,
  ...
  assert.equal(
    ...
    "cost-weighted-full-budget-plus-matched-slot",
  );
});

test("V12 hybrid ranking is transitive and independent of input order", () => {
  const entries = [
    {
      id: "passenger",
      cost: 17,
      ...
      counterfactualWinGain: 0,
      counterfactualRobustWinGain: -0.0335,
      bestDeck: { ids: ["a", "b", "c", "passenger", "e"], totalCost: 94, remainingCost: 6 },
    },
    {
      id: "papua-like",
      cost: 26,
      ...
      counterfactualWinGain: 0.0417,
      counterfactualRobustWinGain: 0.0088,
      bestDeck: { ids: ["a", "b", "c", "papua-like", "e"], totalCost: 100, remainingCost: 0 },
    },
    {
      id: "cheap-defense",
      cost: 19,
      ...
      bestDeck: { ids: ["a", "b", "c", "cheap-defense", "e"], totalCost: 99, remainingCost: 1 },
    },
  ];

  const expected = ["cheap-defense", "papua-like", "passenger"];
  assert.deepEqual(rankMetagameV12Characters(entries).map((entry) => entry.id), expected);
  assert.deepEqual(rankMetagameV12Characters([entries[2], entries[0], entries[1]]).map((entry) => entry.id), expected);
  assert.deepEqual(rankMetagameV12Characters([entries[1], entries[2], entries[0]]).map((entry) => entry.id), expected);
});
