...
対戦環境評価はGitHub Actions上で実行し、途中経過と評価結果をmetagame-resultsブランチへ保存します。GitHubホステッドランナーは実行時間に上限があるため、標準では1回につき1区画だけ計算して確実に再開します。
...
- 手動実行ではmax_tasksを1のままにします。1区画の完了後、状態・JSON評価結果がmetagame-resultsへpushされます。
- max_tasksを増やすと1回で連続して処理します。GitHub Actionsのジョブ上限を超えない値だけを指定してください。
- 2時間ごとのスケジュール実行も設定済みです。計算中に新しい実行は重複せず、同じ環境評価は同時に1本だけ動きます。
