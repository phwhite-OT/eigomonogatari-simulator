...
V8.2は、環境候補の記載順や個別の採用率を推測しません。各候補を等しく残しつつ、同じ候補を含む複数の合法な完成デッキを作り、72通りの5対5盤面で比較します。候補キャラを入れる側も、似た相方だけを残さず、異なる完成デッキを4本まで実戦比較します。

攻撃・防御・蘇生・色変を人工的なデッキ型へ分けることはしません。継続は独立した評価軸ではなく、実戦内で攻撃・防御・蘇生・色変の効果が次キャラまで残り、勝敗へ与えた結果としてだけ評価されます。

V8.2のチェックポイントは `metagame-v8.2-results` ブランチと `reports/metagame-ratings-v8.2/<input>/` に保存されます。V8の結果とは混在させず、V8.2で縛りが完了したものから公開データを更新します。
