...
## ブラウザの「最軽装を探索」ボタンから使う

最初にPCのターミナルで次を実行し、そのターミナルを開いたままにします。

```powershell
npm run serve:lightest:local
```

その後は、いつものブラウザ画面で「最軽装を探索」を押すだけです。画面は `http://127.0.0.1:41773` のローカル計算機を自動検出し、論理CPU数から1を引いた数の Node.js Worker で計算します。探索はPC内で完結し、GitHub Actionsは使用しません。

計算を中止しても、コスト帯・完了shard・発見済みの上限は `.cache/lightest/` に残ります。同じ条件ならキャッシュを返し、途中で止めた条件なら次回のボタン操作で続きを再開します。

ローカル計算機が起動していない場合、画面に起動コマンドが表示されます。ブラウザからNode.jsプロセスを直接起動することはできないため、最初の一度だけこの常駐サービスの起動が必要です。
